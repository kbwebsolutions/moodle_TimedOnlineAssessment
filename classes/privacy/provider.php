...
namespace assignsubmission_timedonline\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('assignsubmission_timedonline', $detail, 'privacy:metadata:tablepurpose');
    ...
    public static function export_submission_user_data(assign_plugin_request_data $exportdata) {
        ...
        $plugin = $assign->get_plugin_by_type('assignsubmission', 'timedonline');
        $submission = $exportdata->get_pluginobject();
        $editortext = $plugin->get_editor_text('timedonline', $submission->id);
        $context = $exportdata->get_context();
        if (!empty($editortext)) {
            $submissiontext = new \stdClass();
            $currentpath = $exportdata->get_subcontext();
            $currentpath[] = get_string('privacy:path', 'assignsubmission_timedonline');
            $submissiontext->text = writer::with_context($context)->rewrite_pluginfile_urls($currentpath,
                    'assignsubmission_timedonline', 'submissions_timedonline', $submission->id, $editortext);
            writer::with_context($context)->export_area_files($currentpath, 'assignsubmission_timedonline',
            'submissions_timedonline', $submission->id)->export_data($currentpath, $submissiontext);
    ...
    public static function delete_submission_for_context(assign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, 'assignsubmission_timedonline',
                ASSIGNSUBMISSION_TIMEDONLINE_FILEAREA);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_timedonline', ['assignment' => $requestdata->get_assignid()]);
    }
    ...
    public static function delete_submission_for_userid(assign_plugin_request_data $deletedata) {
        ...
        // Delete related files.
        $fs = get_file_storage();
        $fs->delete_area_files($deletedata->get_context()->id, 'assignsubmission_timedonline',
         ASSIGNSUBMISSION_TIMEDONLINE_FILEAREA,
                $submissionid);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_timedonline', ['assignment' => $deletedata->get_assignid(),
    ...
    public static function delete_submissions(assign_plugin_request_data $deletedata) {
        ...
        $fs->delete_area_files_select($deletedata->get_context()->id,
                'assignsubmission_timedonline', ASSIGNSUBMISSION_TIMEDONLINE_FILEAREA, $sql, $params);

        $params['assignid'] = $deletedata->get_assignid();
        $DB->delete_records_select('assignsubmission_timedonline', "assignment = :assignid AND submission $sql", $params);
    }
}
